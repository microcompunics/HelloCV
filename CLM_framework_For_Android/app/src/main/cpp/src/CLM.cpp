...
#include "../include/CLM.h"
#include "../include/CLM_utils.h"
...
void CLM::Read_CLM(string clm_location)
{
	...
	{
		LOGD("Couldn't open the CLMModel model file aborting");
		//cout << "Couldn't open the CLMModel model file aborting" << endl;
		//cout.flush();
		return;
	}
	...
	string root = getParentPath(clm_location);
...
void CLM::Read(string main_location)
{
	...
	//cout << "Reading the CLMModel landmark detector/tracker from: " << main_location << endl;
	LOGD("Reading the CLMModel landmark detector/tracker from: %s",main_location.c_str());
	ifstream locations(main_location.c_str(), ios_base::in);
	if(!locations.is_open())
	{
		LOGD("Couldn't open the model file, aborting");
	...
	string root = getParentPath(main_location);
	// The main file contains the references to other files
	while (!locations.eof())
	{ 
		getline(locations, line);
		...
		{ 
			//cout << "Reading the CLMModel module from: " << location << endl;
			LOGD("Reading the CLMModel module from: %s",location.c_str());
			// The CLMModel module includes the PDM and the patch experts
			Read_CLM(location);
		}
...
bool CLM::Fit(const Mat_<uchar>& im, const Mat_<float>& depthImg, const std::vector<int>& window_sizes, const CLMParameters& clm_parameters)
{
	...
	for(int scale = 0; scale < num_scales; scale++)
	{
		...
		{
			LOGD("Detection too small for CLMModel");
			//cout << "Detection too small for CLMModel" << endl;
			return false;
		}
	}
...
bool CLM::RemoveBackground(Mat_<float>& out_depth_image, const Mat_<float>& depth_image)
{
	...
	{
		LOGD("Face estimate is too close to the edge, tracking failed");
	...
	{
		LOGD("No depth signal found in foreground, tracking failed");
...
Mat_<double> CLM::GetShape(double fx, double fy, double cx, double cy) const
{
	...
	// this is described in the paper in section 3.4 (equation 10) (of the CLMModel-Z paper)
	for(int i = 0; i < n; i++)
	{
