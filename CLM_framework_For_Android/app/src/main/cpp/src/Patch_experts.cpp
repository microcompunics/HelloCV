...
#include "../include/Patch_experts.h"
							 ...
							 const PDM& pdm, const Vec6d& params_global, const Mat_<double>& params_local, int window_size, int scale)
{
	...
	// Indicates the legal pixels in a depth image, if available (used for CLMModel-Z area of interest (window) interpolation)
	Mat_<uchar> mask;
...
void Patch_experts::Read(vector<string> intensity_svr_expert_locations, vector<string> depth_svr_expert_locations, vector<string> intensity_ccnf_expert_locations)
{
	...
	{		
		...
		LOGD("Reading the intensity SVR patch experts from: %s....",location.c_str());
	...
	{		
		...
		LOGD("Reading the intensity CCNF patch experts from: %s....",location.c_str());
	...
	{
		LOGD("Intensity and depth patch experts have a different number of scales, can't read depth");
	...
	{		
		...
		LOGD("Reading the depth SVR patch experts from: %s....",location.c_str());
		...
		{
			LOGD("Intensity and depth patch experts have a different scales, can't read depth");
		...
		for(int view = 0; view < num_views_depth; ++view)
		{
			...
			{
				LOGD("Intensity and depth patch experts have different visibilities or centers");
...
void Patch_experts::Read_SVR_patch_experts(string expert_location, std::vector<cv::Vec3d>& centers, std::vector<cv::Mat_<int> >& visibility, std::vector<std::vector<Multi_SVR_patch_expert> >& patches, double& scale)
{
	...
	{
		LOGD("Can't find/open the patches file");
